	...
	while True:
		try:
			...
			if (FC in [1, 2, 3, 4]):  # Read Inputs or Registers
				...
				print("TID = %d, ID= %d, Fun.Code= %d, Address= %d, Length= %d, Data= %s" % (TID0 * 256 + TID1, ID, FC, ADR, LEN, str(DATA)))
				...
				print("TID = %d, ID= %d, Fun.Code= %d, Address= %d, Length= %d, Bytes= %d" % (TID0 * 256 + TID1, ID, FC, ADR, LEN, BYT))
				if FC == 5 or FC == 15:
				...
				print("Funtion Code ", FC, "Not Supported")
				exit()
